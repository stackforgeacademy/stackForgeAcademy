...
export default function Hero() {
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center text-center px-6 pt-32 pb-10 overflow-hidden">
      {/* Background Effects */}
      ...
      <motion.h1
        ...
        className="text-[clamp(30px,5vw,68px)] font-extrabold leading-[1.12] tracking-tight max-w-[900px] text-[var(--text-contrast)]"
      >
      ...
      <motion.p
        ...
        className="mt-4.5 text-[clamp(13px,1.6vw,18px)] text-slate-400 max-w-[640px]"
      >
      ...
      >
        {['Full Stack Dev with GenAI', 'DSA', 'System Design', 'Deployment'].map((label) => (
          <span 
            key={label} 
            className="border rounded-full px-4 py-1.25 text-[clamp(11px,1.2vw,13px)] font-semibold backdrop-blur-md transition-transform hover:scale-105 cursor-default text-cyan-400 border-cyan-500/30 bg-cyan-500/10 shadow-[0_0_15px_rgba(34,211,238,0.1)]"
          >
            {label}
          </span>
        ))}
      ...
      >
        <a href="#apply" className="inline-flex items-center gap-2 bg-gradient-to-br from-[#2f3080] to-[#2a65c5] text-white px-7 py-3.5 rounded-xl text-[15px] font-semibold transition-all hover:-translate-y-0.5 hover:shadow-[0_8px_40px_rgba(99,102,241,0.55)] shadow-[0_4px_30px_rgba(99,102,241,0.4)]">
          <Zap size={16} strokeWidth={2.5} />
          Apply for Selection
        </a>
        <button
          onClick={generateCurriculum}
          className="inline-flex items-center gap-2 bg-transparent text-slate-400 border border-white/75 px-6 py-3.5 rounded-xl text-sm font-medium transition-colors hover:border-blue-600 hover:text-white"
        >
      ...
      <motion.div
        ...
        className="grid grid-cols-2 gap-x-10 gap-y-8 md:flex md:flex-row md:gap-16 mt-14 max-w-fit mx-auto"
      >
        {[
          ...
          <div key={stat.label} className="text-center">
            <div className={`text-[clamp(22px,3vw,36px)] font-extrabold ${stat.color || 'text-blue-bright'}`}>
              {stat.num.includes('*') ? (
                <>
                  {stat.num.replace('*', '')}
            ...
            <div className="text-[12px] text-slate-400 mt-0.5 tracking-widest uppercase">
              {stat.label.includes('*') ? (
                <>
